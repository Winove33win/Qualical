...
export default function QualicalLanding() {
  // THEME – ajuste com a paleta da marca
  const theme = {
    "--color-bg": "#ffffff",
    "--color-surface": "#ffffff",
    "--color-primary": "#2E5AAC",      // Azul
  ...
  // UTIL – placeholder de imagem como SVG inline (sempre carrega)
  const ph = (w: number, h: number, label = "Imagem") =>
  ...
  const S = useMemo(() => ({
    ...
    headerWrap: { position: "sticky" as const, top: 0, zIndex: 50, background: "rgba(255,255,255,.95)", backdropFilter: "blur(14px)", borderBottom: "1px solid #e2e8f0" },
    ...
    hero: { marginTop: 24, borderRadius: "var(--radius-xl)", overflow: "hidden", position: "relative" as const },
    heroBg: { position: "absolute" as const, inset: 0, background: `linear-gradient(120deg, var(--color-primary-50), var(--color-accent-50))` },
    heroInner: { position: "relative" as const, display: "grid", gridTemplateColumns: "1.1fr .9fr", gap: 24, padding: 32, alignItems: "center" },
    ...
    cardBody: { padding: 16 },
    pill: { display: "inline-block", fontSize: 12, fontWeight: 900, color: "#064e3b", background: "var(--color-accent-50)", padding: "6px 10px", borderRadius: 999, letterSpacing: .4, textTransform: "uppercase" as const },
    list: { margin: 0, paddingLeft: 18 },

    // Carrossel
    carousel: { position: "relative" as const, borderRadius: "var(--radius-xl)", overflow: "hidden", border: "1px solid #e5e7eb", boxShadow: "var(--shadow)", background: "#fff" },
    track: { display: "flex", transition: "transform .5s ease" },
    slide: { minWidth: "100%", padding: 0 },
    arrow: { position: "absolute" as const, top: "50%", transform: "translateY(-50%)", background: "rgba(255,255,255,.9)", border: "1px solid #e2e8f0", boxShadow: "var(--shadow)", borderRadius: 12, width: 44, height: 44, display: "grid", placeItems: "center", cursor: "pointer" },
    dotWrap: { position: "absolute" as const, left: "50%", transform: "translateX(-50%)", bottom: 12, display: "flex", gap: 8 },
    dot: (active: boolean) => ({ width: 10, height: 10, borderRadius: 10, background: active ? "var(--color-accent)" : "#cbd5e1", border: "1px solid #94a3b8" }),
    ...
    footBottom: { marginTop: 22, display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap" as const, gap: 12, color: "var(--color-muted)" },

    // responsive
    "@media (max-width: 980px)": { heroInner: { gridTemplateColumns: "1fr" } },
  }), []);

  // DATA – ampliado a partir do material fornecido
  const services = [
  ...
  const dragging = useRef<{startX:number;scrollX:number}|null>(null);
    ...
    <div style={{ ...S.page, ...theme }}>
      ...
      <section style={{ ...S.container, ...S.hero }}>
        ...
        <div style={S.heroInner}>
          ...
          <div style={S.carousel}>
            ...
            >
              {slides.map((s) => (
                <figure key={s.title} style={S.slide as React.CSSProperties}>
      ...
      <section id="servicos" style={{ ...S.container, ...S.section }}>
        ...
        <div style={S.gridAuto}>
          {services.map((s) => (
            <article key={s.title} style={S.card as React.CSSProperties}>
      ...
      <section id="auditorias" style={{ ...S.container, ...S.section }}>
        ...
        <div style={S.gridAuto}>
          {extras.map((e) => (
            <article key={e.title} style={S.card as React.CSSProperties}>
      ...
      {/* TREINAMENTOS – Jornada com detalhes */}
      <section id="treinamentos" style={{ ...S.container, ...S.section }}>
        ...
        <div style={S.gridAuto}>
          {steps.map((st) => (
            <div key={st.n} style={{ padding: 18, background: "#fff", border: "1px solid #e2e8f0", borderRadius: "var(--radius-lg)", boxShadow: "var(--shadow)", position: "relative" as const }}>
      ...
      <section id="contato" style={{ ...S.container, ...S.section }}>
        ...
        <div style={{ display: "grid", gap: 18, gridTemplateColumns: "1fr 1fr" }}>
          <form onSubmit={(e) => { e.preventDefault(); alert('Mensagem enviada! (demo front)'); }} style={{ ...S.card as React.CSSProperties, padding: 18 }}>
            ...
            <label style={{ display: "grid", gap: 8, marginBottom: 10 }}>
              <span>Mensagem</span>
              <textarea required placeholder="Conte sua necessidade" style={{ width: "100%", height: 120, borderRadius: 12, border: "1px solid #e2e8f0", padding: "12px 14px", resize: "vertical" as const }} />
            </label>
            <button style={S.ctaBtn}>Enviar</button>
          </form>
          <div style={{ ...S.card as React.CSSProperties, padding: 18 }}>
